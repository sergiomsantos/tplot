...
COLORS  = cycle([Colors.BLUE, Colors.GREEN, Colors.RED, Colors.PURPLE])
MARKERS = cycle('ox+-.')
...
class TPlot(object):
    ...
    def get_canvas(self):
        ...
        for n,dataset in enumerate(self.datasets):
            ...
            k = len(label)
            legend = Colors.BOLD + color + label + ' ' + marker + Colors.ENDC
        ...
        # canvas.append([xticks[0][0]*' ' + labels.rstrip()])
...
#     plot = TPlot(tsize.columns, tsize.lines)
#     plot.plot(x, x)
#     plot.plot(x,  np.sin(x)*np.cos(x)*x, label='pai')
#     plot.plot(x, -np.sin(x)*np.cos(x)*x, label='mae')
#     plot.plot(x, 5*np.cos(x))

#     print(plot)
#     print(plot.get_legend())
...
#     # print(np.c_[x,np.sin(x)*np.cos(x)*x,5*np.cos(x)])
#     # np.savetxt('data.txt',np.c_[x,np.sin(x)*np.cos(x)*x,5*np.cos(x), np.exp(x),10*np.ones_like(x)])
...
#     # print(tsize.columns, tsize.lines)
...
#     group.add_argument('--stdout', action='store_true', help='print to console rather than using a matplotlib window')
